...
import { authAPI, appointmentsAPI, servicesAPI, usersAPI } from '../services/api'
...
function AdminDashboard() {
  ...
  const loadDashboard = async () => {
    try {
      ...
      // Get all stats
      const [appointments, servicesData, users] = await Promise.all([
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-50 via-purple-50 to-pink-50">
      ...
      <main className="container mx-auto px-3 sm:px-4 py-4 md:py-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6 mb-6 md:mb-8">
          ...
          <div className="bg-gradient-to-br from-indigo-500 to-indigo-600 rounded-lg shadow-lg p-6 text-white transform hover:scale-105 transition-all duration-300 animate-slide-up" style={{ animationDelay: '0.4s' }}>
            <div className="flex items-center justify-between">
              <div>
                <h3 className="text-indigo-100 text-xs sm:text-sm font-medium mb-2">
                  Total Users
                </h3>
                <p className="text-3xl sm:text-4xl font-bold">
                  {stats.users}
        ...
        <div className="bg-white rounded-lg shadow p-6 mb-8">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <div className="text-center p-4 bg-purple-50 rounded-lg">
              <p className="text-2xl font-bold text-purple-600">{stats.userBreakdown.admin}</p>
              <p className="text-sm text-gray-600 mt-1">Admins</p>
            </div>
            <div className="text-center p-4 bg-blue-50 rounded-lg">
              <p className="text-2xl font-bold text-blue-600">{stats.userBreakdown.manager}</p>
              <p className="text-sm text-gray-600 mt-1">Managers</p>
            </div>
            <div className="text-center p-4 bg-green-50 rounded-lg">
              <p className="text-2xl font-bold text-green-600">{stats.userBreakdown.receptionist}</p>
              <p className="text-sm text-gray-600 mt-1">Receptionists</p>
            </div>
            <div className="text-center p-4 bg-orange-50 rounded-lg">
              <p className="text-2xl font-bold text-orange-600">{stats.userBreakdown.client}</p>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6 mb-6 md:mb-8">
          ...
          <div
            onClick={() => navigate('/staff/recent-users')}
            className="bg-white rounded-lg shadow-lg p-6 hover:shadow-xl transform hover:scale-105 transition-all duration-300 cursor-pointer animate-slide-up"
            style={{ animationDelay: '0.4s' }}
          >
            <div className="flex items-center justify-between mb-3 md:mb-4">
              <h3 className="text-base md:text-lg font-semibold text-gray-900">Recent Users</h3>
              <svg className="w-6 h-6 md:w-8 md:h-8 text-indigo-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197M13 7a4 4 0 11-8 0 4 4 0 018 0z" />
              </svg>
            </div>
            <p className="text-2xl md:text-3xl font-bold text-indigo-600 mb-2">{stats.users}</p>
            <p className="text-xs md:text-sm text-gray-500">View all users →</p>
          </div>
        </div>
